...
def run_once(seed: int, splits: dict, cfg: dict) -> dict:
    ...
    # ── Neural Demand (habit, CF) — HabitND with precomputed xbar ────────
    ...
    KW = dict(
        ...
        ff=feat_shared, theta=th_sh,          # Linear Demand (Shared)
        nirl=nirl_m,
        nirl_cf=nirl_cf_m,
        mdp_cf=mdp_cf_m,
        mdp_e2e=nd_hab_m,
    ...
    def _xb(paper_name):
        ...
        if "CF" in paper_name and "habit" in paper_name:
            return xb_te   # precomputed EWMA for HabitND-CF
        if "habit" in paper_name:
            return xb_e2e_te
        return xb_te   # for HabitND (non-E2E)
    ...
    def _kw_extra(paper_name):
        if paper_name == "Linear Demand (GoodSpec)":
            return {"ff": feat_good_specific, "theta": th_gs}
        if paper_name == "Linear Demand (Orth)":
